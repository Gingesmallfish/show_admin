<template>
  <el-card shadow="never" class="border-0">
      ...
      <el-row :gutter="20">
        <el-col :span="8" :offset="0">
          <el-form-item label="关键字">
            <el-input v-model="searchFrom.keyword" placeholder="要搜索的关键字" clearable></el-input>
          </el-form-item>
        </el-col>
    ...
    <el-table :data="tableData" stripe style="width: 100%" v-loading="loading">
      ...
      <el-table-column prop="所属管理员" align="center">
        <template #default="{ row }">
          {{ row.role?.name || '_' }}
        </template>
      </el-table-column>
      <el-table-column label="状态" width="120">
        <template #default="{ row }">
          <el-switch :model-value="row.status" :active-value="1" :inactive-value="0" :loading="row.statusLoading"
            @change="handleStatusChange($event, row)" :disabled="row.super == 1"></el-switch>
        </template>
      </el-table-column>
...
function getData(p = null) {
  ...
  loading.value = true
  getManagerList(currentPage.value, searchFrom)
    .then(res => {
